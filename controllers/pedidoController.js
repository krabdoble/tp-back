...
const getAllPedidoItems = async (req, res = response) => {
    try {
        const pedido = await Pedido.findAll({ include: Producto });
        res.json(pedido);
      } catch (error) {
...
const createPedidoList = async (req, res = response) => {
    const { items, suma_total, fecha_entrega } = req.body;
    try {
        
      const pedido = await Promise.all(items.map(item=>Pedido.create({
        cantidad: item.cantidad, talle: item.talle, precio_unidad: item.precio_unidad, precio_cantidad: item.precio_cantidad, suma_total: suma_total, fecha_entrega: fecha_entrega, productoId:item.productoId
      } 
    )));
...
const getPedidoById = async (req, res = response) => {
  try {
      const pedido = await Pedido.findByPk(req.params.id,{ include: Producto });
